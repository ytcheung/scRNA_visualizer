...
observeEvent(input$vizDePlot,{
  withProgress(message = "Processing , please wait",{
    #Reset variables to release memory
    plotDE <<- list()
    plot <- NA
    markers <- list()
    ...
    groups <- data.frame(group1 = as.factor(sce[[input$dePlot.group1]]), row.names = colnames(sce[[COL_EXPRESSION]]), N = colnames(sce[[COL_EXPRESSION]]))
    ...
    plot <- DrawHeatmap(groups, annogenes, exp[row.names(gene_list),], show, cluster_cell_method, cluster_gene_method, 
